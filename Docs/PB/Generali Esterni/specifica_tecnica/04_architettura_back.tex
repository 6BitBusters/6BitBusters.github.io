...
L’architettura del backend adotta lo stile modulare fornito da NestJS, con moduli principali che gestiscono le funzionalità chiave dell'applicazione e moduli di supporto che forniscono risorse necessari a quelli precedenti. I moduli funzionali sono organizzati secondo un approccio a layer, comprendente controller, servizi e repository.\\
...
\begin{itemize}
    ...
    \item \texttt{IRepository}: Interfaccia per la gestione della cache. Definisce le operazioni fondamentali:
    \begin{itemize}
...
Il \texttt{RepositoryModule} raggruppa i provider e i componenti che gestiscono la logica di persistenza dei dati, come il \texttt{CacheRepository}. Questo modulo fornisce un'interfaccia coerente per la gestione della cache. Il \texttt{CacheRepository} è responsabile della persistenza temporanea dei dati, utilizzando \texttt{Memcached} tramite il client \texttt{memjs}, per ottimizzare le performance dell'applicazione.
...
\begin{itemize}  
  ...
  \item \textbf{Repository (Persistence Layer):} il repository gestisce l'accesso ai dati, separando la logica di business dalla gestione della persistenza. Si occupa di salvare e recuperare i dati in cache, garantendo che il resto dell'applicazione non dipenda dai dettagli di implementazione della persistenza. Il repository fa parte di un modulo separato (\texttt{RepositoryModule}), che viene esportato nel modulo che necessita di salvare i dati. Questo approccio facilita la gestione, l'eventuale integrazione in altri moduli e la sostituzione del sistema di persistenza.
...
\begin{itemize}
  ...
  \item \textbf{Decorator (con \texttt{@Controller} e \texttt{@Get}):}  agiscono come un "decorator" per assegnare funzionalità specifiche a classi e metodi, ma non implementano il pattern decoratore nel senso tradizionale. \texttt{@Controller} definisce una classe come controller, mentre \texttt{@Get} mappa un metodo a una rotta HTTP GET. Questo approccio consente di semplificare la gestione delle richieste senza aggiungere logica complessa;
