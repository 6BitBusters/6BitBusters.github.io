...
\subsubsection{Scopo}
Lo scopo è creare uno standard per la gestione del ciclo di vita di ogni documento.
Il ciclo di vita di ogni documento è così composto:
\begin{itemize}
...
\subsubsubsection{Template standard} \\
...
\noindent \textbf{Terminologia inglese} \\
...
\begin{itemize}
      ...
      \item Ogni acronimo deve essere inserito nel \textit{Glossario v2.0.0}, accompagnato da una
            ...
            comprensione.
\end{itemize}
\subsubsubsection{Strumenti per la stesura}
...
\subsubsubsection{Documentazione interna}
            ...
            \begin{itemize}
                  \item \textbf{Informazioni generali}: contiene i dettagli dell'incontro, nello specifico:
                        \begin{itemize}
                              ...
                              \item Partecipanti.
                        \end{itemize}
                  \item \textbf{Ordine del giorno}: breve descrizione narrativa di cosa è stato trattato in quell'incontro, che descrive gli argomenti trattati nella riunione;
...
\subsubsubsection{Documentazione esterna}
...
ciclo di vita. Questo processo si applica sia alla documentazione che al codice 
sorgente.
\subsubsection{Scopo}
Lo scopo di questo processo è garantire che ogni modifica sia tracciabile, controllata e approvata 
prima di essere integrata nel prodotto finale.
\subsubsection{Attività}
...
\subsubsubsection{Branching}\label{inf:branch}
...
\begin{itemize}
      ...
      \item \textbf{Branch derivati}: sono branch utilizzati per aggiungere modifiche e aggiornare un documento o una parte di codice che poi dovrà essere verificata
            ...
            \begin{itemize}
                  ...
                  \item \textbf{Per tutto il resto della documentazione}:
                        ...
                        \end{center}
            \end{itemize}
            Per quanto riguarda il codice sorgente, i nomi dei branch derivati dipendono dalla parte del progetto su cui si sta lavorando e seguono la seguente convenzione:                  
            \begin{itemize}
                  \item \textbf{Per il codice frontend}:
                        \begin{center}
                              \textbf{code/frontend-[ID-ISSUE]}
                        \end{center}
                  \item \textbf{Per il codice backend}:
                        \begin{center}
                              \textbf{code/backend-[ID-ISSUE]}
                        \end{center}
            \end{itemize}
            dove:
            \begin{itemize}
      ...
      \item \textbf{Branch di hotfix}: sono branch dedicati all'hotfix e quindi a correzioni minime, sia per la documentazione che per il codice. Valgono le stesse regole dei
            ...
            \begin{itemize}
                  \item \textbf{Per la documentazione}:
                        \begin{center}
                              \textbf{hotfix/[NOME-DOCUMENTO]}
                        \end{center}
                  \item \textbf{Per il codice frontend}:
                        \begin{center}
                              \textbf{hotfix/frontend}
                        \end{center}
                  \item \textbf{Per il codice backend}:
                        \begin{center}
                              \textbf{hotfix/backend}
                        \end{center}
            \end{itemize}        
\end{itemize}
...
\begin{itemize}
      ...
      \item \textbf{code/frontend} oppure \textbf{code/backend} $\rightarrow$ il codice deve essere verificato e successivamente viene eliminato il branch derivato.
\end{itemize}
...
\subsubsubsection{Commit}\label{inf:comm}
...
Dopo l'approvazione di una pull request tutti i commit relativi al documento oppure al codice
...
Per il codice frontend:
\begin{center}
      \textbf{Update: frontend-[ID-ISSUE]}
\end{center}

Per il codice backend:
\begin{center}
      \textbf{Update: backend-[ID-ISSUE]}
\end{center}
...
\begin{itemize}
      ...
      \item \textbf{ID-ISSUE}: indica il numero identificativo associato alla issue relativa alla modifica del documento o del codice.;
...
\subsubsubsection{Pull request}\label{inf:pr}
...
\begin{itemize}
      \item \textbf{Title}:
            \begin{itemize} 
                  \item Per la documentazione: \textbf{[NOME-DOCUMENTO]-[ID-ISSUE]}; 
                  \item Per il codice frontend: \textbf{Frontend-[ID-ISSUE]};
                  \item Per il codice backend: \textbf{Backend-[ID-ISSUE]}; 
            \end{itemize}
      ...
      \item \textbf{Gli assegnatari}: coloro che hanno anche il compito di apportare le modifiche necessarie al documento o al codice;
...
\subsubsubsection{Automazioni}\label{inf:automaz}
...
\noindent \textbf{Automazioni realizzate per la documentazione} \\
...
\noindent \textbf{Automazioni realizzate per la codifica} \\
L'automazione per la parte di codifica ha lo scopo di garantire che ogni modifica al codice sorgente rispetti determinati 
standard qualitativi e non introduca errori. Le automazioni sono state realizzate tramite GitHub Actions e si attivano in 
occasione di pull request verso i branch dedicati al frontend o al backend. In particolare, sono stati creati i seguenti 
workflow:
\begin{itemize}
      \item \texttt{pr\_frontend\_check.yml}: esegue una serie di verifiche sul codice frontend contenuto nella cartella \texttt{3Dataviz/frontend}; 
      \item \texttt{pr\_backend\_check.yml}: analogamente, esegue i controlli sulla parte backend situata nella cartella \texttt{3Dataviz/backend}. \\

      Per entrambi vengono eseguite le seguenti operazioni:      \begin{itemize} 
            \item installazione delle dipendenze; 
            \item verifica della formattazione del codice; 
            \item esecuzione dell'analisi statica del codice; 
            \item avvio dei test automatici; 
            \item build del progetto. 
      \end{itemize}
\end{itemize}      

I workflow implementati sono fondamentali per mantenere l’integrità del codice. Per questo motivo, il repository è stato configurato affinché non sia 
possibile effettuare il merge di una pull request se i relativi controlli automatici non hanno esito positivo. Inoltre, è raccomandata l’esecuzione locale 
degli stessi comandi di verifica prima dell’apertura di una pull request, per ridurre il tempo di revisione e prevenire errori banali.
\vspace{1em}

\noindent \textbf{Come aggiungere un'action} \\
...
\noindent \textbf{Come aggiornare il sito} \\
...
Docker viene utilizzato per la creazione di immagini e container isolati per facilitare e velocizzare lo sviluppo dell'applicazione. 
Esso garantisce inoltre portabilità e coerenza tra ambienti.
\vspace{1em}

\noindent \textbf{Docker compose} \\
Sono stati creati 2 file per la creazione e l'avvio automatico dei container:
\begin{itemize}
...
\subsubsection{Descrizione}
Il processo di accertamento della qualità controlla il raggiungimento degli obiettivi di qualità che il team si
è prefissato di raggiungere e le metriche utilizzate per misurare tale raggiungimento.\\ Il controllo della qualità consite nella misurazione e
nell'analisi degli indicatori di interesse, valutando il grado di raggiungimento
...
processi adottati e del prodotto software realizzato, al fine di soddisfare le aspettative
degli stakeholder e i requisiti del progetto. Ciò avviene tramite le attività
di pianificazione della qualità, il controllo della qualità e il miglioramento
continuo. 
\subsubsection{Attività}
\subsubsubsection{Ciclo di Deming}
...
\subsection{Verifica}
\subsubsection{Descrizione}
Il processo di verifica viene applicato costantemente sia durante la stesura della
...
Quando vi sarà la necessità di modificare o aggiungere nuove informazioni ad una sezione di
...
in esecuzione. I test vengono progettati per garantire che il codice
...
\subsubsection{Descrizione}
La validazione è il processo attraverso cui il fornitore dimostra che il prodotto sviluppato soddisfa tutti i requisiti utente. 
Si svolge al termine dello sviluppo e rappresenta una fase fondamentale per confermare che il sistema risponda alle esigenze del cliente.

\subsubsection{Scopo}
Lo scopo di tale processo è verificare che il prodotto corrisponda alle specifiche concordate, 
sia pronto per l'utilizzo previsto e risponda efficacemente alle aspettative del committente.

\subsubsection{Attività}
\subsubsubsection{Test di accettazione}
I test di accettazione hanno l'obiettivo di verificare che il prodotto finale sia conforme ai requisiti e possa essere rilasciato.  
Vengono definiti insieme al proponente durante l'analisi dei requisiti e rappresentano il criterio principale per approvare il sistema.  
Sono eseguiti in un ambiente che riproduce le condizioni operative reali.