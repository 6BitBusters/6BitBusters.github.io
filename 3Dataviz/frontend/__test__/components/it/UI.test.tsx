...
describe("UI", () => {
    ...
    describe("CameraResetButton",()=>{
        ...
        it("Il bottone di reset camera deve eseguire un`azione di tween sulla telecamra e riportarla alla posizione iniziale", async () => {
          const mockCustomCanvasRef = React.createRef<{ resetCamera: () => void }>();
          const mockControls = {
            current: {
              object: {
          ...
          render(
            <Provider store={mockStore(CreateMockRootState())}>
              ...
              />
              <UI datasetName="" resetCamera={() => mockCustomCanvasRef.current?.resetCamera()} />
            </Provider>,
          );
          ...
          expect(gsapToSpy).toHaveBeenNthCalledWith(2, mockControls.current.target, {
            x: initialTarget[0],
            y: initialTarget[1],
            z: initialTarget[2],
            duration: 1,
          ...
          expect(gsapToSpy).toHaveBeenNthCalledWith(3, mockControls.current.object, {
            zoom: initialZoom,
            duration: 1,
            delay: 0,
            ease: "power2.out",
