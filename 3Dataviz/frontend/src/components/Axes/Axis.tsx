...
function Axis({labels,color,endPoint,placementFunction,labelScale=1}:AxisProp) {
    ...
    return(
        ...
        {labelSprites.map((text, index) => (
            <sprite key={text.text} position={text.position} scale={[8*labelScale,2*labelScale,2*labelScale]} ref={(el) => (spriteRefs.current[index] = el)}>
